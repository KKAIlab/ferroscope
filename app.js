...
export function renderResearchProfile(labId) {
  ...
  const methods = labMethods(labId); const studies = (profile?.majorStudies || []).map((study) => {
    ...
    // The laboratory layer opens with the question the laboratory is pursuing, and the paper layer
    // should read the same way: what was open, what this work added, what it rests on, where it
    // stops. Every one of those sentences already exists in English on the paper record — the
    // profile was showing only the reading status and a link, so the most teachable content in the
    // project was one click away and invisible. The Chinese `study.question` / `study.finding`
    // fields are deliberately NOT used: they are unpublished pending a checked translation, and
    // rendering them here would both break the English surface and publish uncorroborated text.
    const brief = english?.sixtySecond;
    const teaching = brief
      ? `<section class="study-question"><span>What was open</span><strong>${escapeHtml(brief.story)}</strong></section>
         <dl class="study-chain">
           <dt>What this adds</dt><dd>${escapeHtml(brief.advance)}</dd>
           <dt>Rests on</dt><dd>${escapeHtml(brief.evidenceAnchor)}</dd>
           <dt class="limit">Stops at</dt><dd class="limit">${escapeHtml(brief.scope)}</dd>
           ${brief.openQuestion ? `<dt class="open">Still open</dt><dd class="open">${escapeHtml(brief.openQuestion)}</dd>` : ""}
         </dl>`
      : "";
...
<section class="research-block"><p class="reading-layer-kicker">Scale 1 · 60-second question card</p><div class="sixty-grid"><div><span>What was open</span><p>${escapeHtml(sixty.story)}</p></div><div><span>What this adds</span><p>${escapeHtml(sixty.advance)}</p></div><div><span>Rests on</span><p>${escapeHtml(sixty.evidenceAnchor)}</p></div><div><span>Stops at</span><p>${escapeHtml(sixty.scope)}</p></div><div><span>Still open</span><p>${escapeHtml(sixty.openQuestion)}</p></div></div></section>
...
function renderResources() {
  ...
  $("#resourceGrid").innerHTML = resources.map((item) => `<article class="resource-card"><div class="resource-top"><span>${escapeHtml(item.type)}</span><b>${escapeHtml(item.authority)}</b></div><h3>${escapeHtml(item.name)}</h3><p>${escapeHtml(item.description)}</p><div class="resource-caution"><b>Stops at</b>${escapeHtml(item.caution)}</div><div class="resource-bottom"><span>${escapeHtml(item.language)} · checked ${escapeHtml(item.checkedAt)}</span><a href="${safeUrl(item.url)}" target="_blank" rel="noreferrer">Open resource ↗</a></div></article>`).join("");
}
