...
const verificationDepthLabels = { "metadata-checked": "Metadata-checked", "abstract-cross-checked": "Abstract-cross-checked", "archive-derived": "Archive-derived", "full-text-rechecked": "Full-text-rechecked", "raw-data-rechecked": "Raw-data-rechecked" };
...
function depthBadge(paper) {
  ...
  if (sources.some((source) => source.kind === "pubmed" && source.status === "checked")) parts.push("abstract cross-checked");
  const fullText = sources.find((source) => source.kind === "publisher-full-text");
  if (!fullText || fullText.status !== "checked") parts.push("full figures pending");
  return parts.join(" · ");
}
...
export function renderPaperDetail(paperId) {
  ...
  const events = (paper.versionEvents || []).map((event) => `<li><b>${escapeHtml(noticeTypeLabels[event.noticeType] || event.type)}</b> · ${escapeHtml(event.date)}${event.doi ? ` · <a href="${safeUrl(`https://doi.org/${event.doi}`)}" target="_blank" rel="noreferrer">${escapeHtml(event.doi)}</a>` : ""}${event.sourceUrl ? ` · <a href="${safeUrl(event.sourceUrl)}" target="_blank" rel="noreferrer">open the notice ↗</a>` : ""}<span class="event-domains">Affects: ${escapeHtml((event.affectedDomains || []).join(", ") || "domain not recorded")}</span><span class="event-impact">${escapeHtml(conclusionImpactLabels[event.conclusionImpact] || conclusionImpactLabels.unknown)}</span><span>${escapeHtml(event.note)}</span><small>Notice read on ${escapeHtml(event.checkedAt || "a date that was not recorded")}</small></li>`).join("");
  const checkedSources = (verification.sources || []).filter((source) => source.status === "checked");
  const uncheckedSources = (verification.sources || []).filter((source) => source.status !== "checked");
  const sourceRows = (verification.sources || []).map((source) => `<li class="source-row ${escapeHtml(source.status || "unknown")}"><b>${escapeHtml(source.kind)}</b><span>${source.status === "checked" ? `checked${source.checkedAt ? ` on ${escapeHtml(source.checkedAt)}` : ""}` : "not checked in this pass"}</span><span>${escapeHtml((source.scope || []).join(", ") || "no field was read from this source")}</span>${source.url ? `<a href="${safeUrl(source.url)}" target="_blank" rel="noreferrer">open ↗</a>` : ""}</li>`).join("");
...
function renderFreshness() {
  ...
  const fullTextRechecked = state.papers.filter((paper) => paper.verificationDepth === "full-text-rechecked" || paper.verificationDepth === "raw-data-rechecked").length;
  ...
  $("#figureStudyCount").textContent = `${figureChains} archive-derived figure chains; ${fullTextRechecked} re-opened from the full text`;
