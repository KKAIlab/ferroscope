...
function renderLabs() {
  ...
  const labs = state.labs
    ...
    .filter((lab) => !term || [lab.pi, lab.institution, lab.region, lab.focus, ...(lab.tags || [])].join(" ").toLowerCase().includes(term))
    .sort((a, b) => (b.relevance || 0) - (a.relevance || 0));
  ...
  $("#labGrid").innerHTML = labs.length ? labs.map((lab, index) => `
    <article class="lab-card">
      ...
      <p class="lab-focus">${lab.focus}</p>
      <div class="lab-tags">${(lab.tags || []).slice(0, 5).map((tag) => `<span>${tag}</span>`).join("")}</div>
      <div class="lab-bottom"><span class="region">${lab.region}</span><span class="watch-state ${state.watchedLabIds.has(lab.id) ? "on" : ""}">● ${state.watchedLabIds.has(lab.id) ? "定向监控" : "官网观察"}</span><a class="visit" href="${lab.website}" target="_blank" rel="noreferrer">LAB 官网 ↗</a></div>
    </article>
...
function bindEvents() {
  ...
  const dialog = $("#freshnessDialog");
  $("#freshnessButton").addEventListener("click", () => dialog.showModal());
  $(".dialog-close").addEventListener("click", () => dialog.close());
  dialog.addEventListener("click", (event) => { if (event.target === dialog) dialog.close(); });
}

async function init() {
  const [labs, curated, live, meta, watchQueries] = await Promise.all([
