...
## Current release status — v0.9.11 (rounds 12–15)

Round 12 froze the corpus and turned cross-file coherence from a report into a standing check. Round 13 closed the acyl axis that the field's consensus figure treats as first-class — and did it entirely inside the already-audited paper set, so nothing here rests on a source this project has not opened.

- **Cross-file coherence is a validator, not a report (round 12).** `scripts/validate-coherence.mjs` checks what no single existing validator owns: whether the layers still agree after a round of edits. Dangling source and event references, a review event paired with a source it did not review, a paper advertising a deeper review state than its registry event recorded, duplicate DOIs and glossary aliases, unattributed papers, two lead laboratories, mechanism edges anchoring DOIs outside `papers-en`, orphan mechanisms, unregistered data files. Two absences are deliberately **not** failures, and the file says so at length: a registered source with no review event is a declared-but-unopened publisher full text, where an attached event would be the forgery; a paper with no lead attribution is one whose senior author runs no laboratory this project tracks, where inventing a lead to fill the slot would be the lie. What the corpus honestly lacks is instead pinned by exact identity in `ACKNOWLEDGED_GAPS`, so widening or quietly closing one of those silences fails the check until it is declared. `scripts/test-coherence.mjs` breaks the corpus 21 ways in a temp-directory copy and asserts each break is rejected.
- **The acyl axis closed from the audited corpus (round 13).** `data/knowledge-network.json` grows from 17 to 18 mechanism nodes and from 23 to 24 evidence-anchored edges, adding **ACSL4–LPCAT3 PUFA esterification** as the enzymatic step that loads arachidonoyl and adrenoyl chains into membrane phospholipids. Its anchor is Kagan 2017 (`10.1038/nchembio.2238`) at `readingDepth: figure-chain` and `verificationDepth: methods-checked` — Fig. 2 audits ACSL4 deletion and Lpcat3 short-hairpin knockdown directly — so unlike round 11's defence arms this node required no new paper at all. The MUFA side deliberately gained **no** node: `mufa-membrane-remodelling` is already defined as the act of writing MUFA into phospholipids, so a second node for SCD1/ACSL3/MBOAT1/2 would have restated it; those enzymes are named in its description instead, and the asymmetry is honest — the PUFA side is anchored at methods-checked depth and the MUFA side at archive-derived.
- **A gap closed and a term added.** `mufa-membrane-remodelling` now carries method links (`epilipidomics`, `genetic-epistasis`), leaving `ACKNOWLEDGED_GAPS` at six mechanisms no method interrogates rather than seven. `data/glossary.json` grows from 35 to 36 trilingual terms with **SCD1**, the last of the acyl-axis enzymes that had no entry; its precision note records that supplying oleic acid restores protection in cells lacking SCD1, so losing SCD1 is not by itself a sensitiser under MUFA-rich conditions.
- **A rendering path that no validator covered.** The reasoning-chain layout lives in `app.js` as `MECHANISM_GROUP`, a table mapping each mechanism to the band it is drawn in. An unmapped node is not dropped — it falls back to the "Disease & therapy" band, stating a position in the causal chain the project never assigned it. Nothing read that table, so adding a mechanism to the data was enough to place it wrongly on screen with every check green. `validate-coherence.mjs` now requires every mechanism to declare its band, rejects a band assigned to an id that is not a mechanism, and **fails closed** if the table is renamed rather than silently skipping the check; three mutation cases cover exactly those breaks. This is the same shape as both entries in `docs/HONESTY-CONTRACTS.md`: a claim reaching the reader through a path the verification layer did not cover.
- **An evidence promotion the tests refused.** Linking the new node to `oxidized-pl-lcms` — the method most characteristic of Kagan 2017 — would have been defensible on the biology and was reverted anyway: that module carries read `assertionScopes`, so a new mechanism link would have inherited **source-checked** state for two edges whose covering scope nobody has read. `test-graph-contract.mjs` caught it. The node is linked through `epilipidomics` and `genetic-epistasis` instead, where the edges stay honestly `recorded-unverified`. Separately, that test's hard-coded method-edge total (74) became a derived invariant — each method-mechanism link contributes exactly one MEASURES and one CANNOT_DISTINGUISH edge — so it now asserts the contract rather than a snapshot that drifts whenever a link is added.
- **The scheduled refresh, which had been failing for six consecutive runs (round 14).** `data/live.json` is a moving PubMed window, not a corpus: `fetchTrackedLabs` requests each laboratory's four most recent ferroptosis papers within one year, so a record leaves the automated layer as soon as its laboratory publishes four newer ones. Three checks had been written against records that happened to be in it — the canonical-merge contract named four ids and required each to be present in **both** the curated and automated layers, the document-classification check named two commentaries, and the timezone display fixture searched the rendered page for a record dated 2025-12-04. When the GPX4 fin-loop paper aged out of the window, the first and third broke, `npm run check` went red inside the refresh workflow, and the site stopped updating. **The asymmetry that hid it: `data/live.json` is committed**, so a local run validates the snapshot in git while CI validates what it just fetched — every local check stayed green throughout. All three contracts are worth keeping and none of them was ever about a particular study, so each is now asserted against a constructed fixture (a temp copy of `data/` with a written `live.json`) and cannot expire. Verified the way the failure had to be verified: `npm run update` for a live fetch — which confirmed the fin-loop record is indeed gone — then a full `npm run check`, green, against both the freshly fetched data and the committed snapshot; each fixture was then mutation-tested (union→overwrite, routes truncated, overlay ignored, formatter reverted to `new Date()`) to confirm it still fails for its own reason.
- **Readability, on a type scale rather than by hand (round 15).** The interface carried 209 font-size declarations across 15 arbitrary values, every one hard-coded in px, with no base size — and **half of the text sat at 9px or below**, including, worst of all, the evidence-boundary notes this project exists to make legible (`.method-boundary`, `.figure-boundary`, `.assay-boundaries`, `.ev-note` were all 9px). A previous pass had raised seven of those declarations by hand and left the other 202. All 209 now resolve to one of eleven semantic steps (`--fs-micro` … `--fs-h1`) defined in rem, so the browser's own text-size setting finally does something — with px it did nothing, which was the accessibility half of "the fonts are too small". The correction is deliberately bottom-heavy: the 8px steps grow 50%, the 30px headings 13%, compressing a range that had been stretched too far. The floor is 12px everywhere, including the graph's orb labels. Raising type broke four layouts that only fitted at the old size, each fixed at its cause rather than by shrinking the text back: the metric figures now align on a baseline instead of a fixed label height, the signal metadata column is sized to its badges, `.review-badge` no longer escapes its column on `width: max-content` under a negative margin, and the mechanism graph gained height and wider labels. Two latent bugs surfaced with them — a `<button>` centring its own wrapped text in the edge-anchor list (`.claim-row` had been corrected for this; the anchor list was missed), and a close-button hover that rotated a symmetric `×` by 90°, which is a state change that renders identically.
- **Expansion that can be undone (round 15).** "Show more" was one-way: the signal list only ever grew by eight, and the terminology list jumped straight from eight entries to all of them with no way back. Both now step by eight in either direction with a paired collapse control, the terminology list included, and collapsing returns the reader to the top of the list it shortened rather than leaving them wherever the shorter document put them. Newly revealed records fade up in a capped stagger, and only the records an expansion actually added — a filter change rebuilds every card, and replaying the entrance for all of them is motion that carries no information. Dialogs open and close on a transition instead of appearing between two frames (`@starting-style` with `allow-discrete`, degrading to the old instant behaviour where unsupported), and the close control became a real button with a hit area, hover and focus ring. All of it is disabled under `prefers-reduced-motion`, decorative loops included.
- **Provenance graph.** The graph holds 264 nodes and 300 edges. By review state: recorded-unverified 77, archive-derived 157, source-checked 66 and independently-rechecked 0. The new mechanism node adds a node, not an edge (`buildGraph` does not consume mechanism→mechanism edges); the eight new edges are the MEASURES/CANNOT_DISTINGUISH pairs of the four new method-mechanism links. `data/source-reviews.json` is unchanged at 79 canonical source records and 47 review events, as is the method layer: 33 of 208 method decision fields are source-checked and 175 remain pending, and 0 datasets are sealed.

## Previous release status — v0.9.10 (round 11)
