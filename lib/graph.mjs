...
// The scope surfaces that belong to a paper's own primary research document, and so can carry
// an experimental claim. Everything outside this set — bibliographic metadata, correction-notice
// text, vendor pages — describes something other than the experiment and may not promote a
// paper-backed experimental edge.
export const EXPERIMENTAL_CONTENT_SURFACES = new Set([
  "abstract-text", "results-text", "body-text", "figure-caption", "figure-panel",
  "methods-text", "supplement-text", "supplement-data", "full-document", "raw-data",
]);
...
export function checkReviewRecord(review, where = "review record") {
  ...
  // Only the canonical registry can mint an independent recheck, because only a resolvable
  // second event can be validated for its prior chain, its different reviewer and its pinned
  // bytes. An inline review copy — the path the notice layer still uses — may not declare this
  // state on its own; without a registry event id there is nothing to validate it against, so
  // it would let the recheck contract be bypassed with a hand-set string.
  if (review.reviewState === "independently-rechecked") {
    require(Boolean(review.reviewEventId), "an independently-rechecked record must carry the registry review event that proves the second reading; an inline copy may not mint this state");
  }
...
export function checkEdgeContract(edge = {}, where = "edge") {
  ...
  // An experimental claim about ferroptosis biology can only be earned by reading the experiment
  // in the paper's own primary research document. A bibliographic metadata record settles who
  // wrote a paper; a correction notice settles what a later notice changed; a vendor page
  // describes a reagent — none of them reports the experiment. Promoting a paper-backed
  // experimental claim off any of those surfaces overstates what was read, so this is an
  // allowlist, not a blocklist: only the paper's own content surfaces qualify, which closes the
  // metadata-record path AND the correction-text path AND any vendor surface at once.
  require(
    !isSourceChecked(edge.reviewState) || edge.provenanceClass !== "paper-backed-experimental" || EXPERIMENTAL_CONTENT_SURFACES.has(edge.scopeSurfaceType),
    `a paper-backed experimental claim must be promoted by a primary-document content surface (${[...EXPERIMENTAL_CONTENT_SURFACES].join(", ")}); ${JSON.stringify(edge.scopeSurfaceType)} is a metadata, correction-notice or vendor surface and reports no experiment`,
  );

  // The promoting scope must be the claim's OWN asserted evidence location. Otherwise a claim
  // about Fig. 3 can be marked source-checked by a genuine reading of Fig. 6 on the same paper —
  // a real reading, but not of what the claim asserts — and the reader is shown "Fig. 3, checked"
  // while nobody checked Fig. 3. When a paper-backed experimental edge names both, they must agree.
  require(
    !(isSourceChecked(edge.reviewState) && edge.provenanceClass === "paper-backed-experimental" && edge.figure && edge.scopeRef && edge.scopeRef !== edge.figure),
    `a paper-backed experimental claim about ${JSON.stringify(edge.figure)} was promoted by scope ${JSON.stringify(edge.scopeRef)}; the promoting scope must be the claim's own asserted evidence location`,
  );
  return problems;
}
...
export function buildGraph({ papers, labs, labsEn, links, methods, network, claims, sourceReviews }) {
  ...
  // Resolves an inline review reference (a `{sourceId, reviewEventId, reviewState}` copy on a
  // paper or a notice) against the one canonical registry. The reviewState, depth, scope, url and
  // version all come from the registry event — never from the inline copy — so raising a private
  // reviewState to "independently-rechecked" without touching the registry cannot promote an edge.
  // The private copy may document the review, but it may not contradict the event: if it out-ranks
  // it, the build fails. `expectedDoi`, when given, requires the cited source to be about that DOI,
  // so one paper cannot borrow another paper's reading; notices pass null because a correction is a
  // different document joined by URL. This is the same join the method routes use.
  const registryReviews = (records, { where, expectedDoi = null }) => (records || []).map((record) => {
    const event = record.reviewEventId ? resolver.event(record.reviewEventId) : null;
    if (record.reviewEventId && !event) problems.push(`${where}: references reviewEventId ${JSON.stringify(record.reviewEventId)} that does not resolve in the registry`);
    if (event && record.sourceId && event.sourceId !== record.sourceId) problems.push(`${where}: cites event ${event.id}, which covers ${event.sourceId}, not ${record.sourceId}`);
    ...
    const registrySource = record.sourceId ? resolver.source(record.sourceId) : null;
    if (expectedDoi && registrySource && registrySource.identifiers?.doi !== expectedDoi) {
      problems.push(`${where}: resolves to a registry source about ${JSON.stringify(registrySource.identifiers?.doi || null)}, not the expected DOI ${JSON.stringify(expectedDoi)}; a paper may not borrow another paper's reading`);
    }
    const checked = Boolean(event && isSourceChecked(event.reviewState));

    // Scopes come from the event, resolved to the registry's own scope objects, so each scope
    // carries its true surface and depth. A figure caption in a manuscript whose Methods were
    // also read inherits figures-legends-checked, not the record's summary maximum.
    const scopes = checked
      ? (event.scopeIds || []).map((id) => {
          const scope = resolver.scope(registrySource?.id, id);
          if (!scope) { problems.push(`${where}: event ${event.id} names scope ${JSON.stringify(id)} that source ${record.sourceId} does not declare`); return null; }
    ...
    const summaryDepth = scopes.length
      ...
      : (checked ? "metadata-checked" : "not-read");

    // The private copy may not out-rank the registry event it points at.
    if (record.reviewState && checked && record.reviewState !== event.reviewState) problems.push(`${where}: declares reviewState ${JSON.stringify(record.reviewState)} but its registry event ${event.id} is ${JSON.stringify(event.reviewState)}; the registry is the authority`);
    if (record.reviewState && !checked && isSourceChecked(record.reviewState)) problems.push(`${where}: declares ${JSON.stringify(record.reviewState)} but no checked registry event backs it`);

    return {
      id: record.id || record.sourceId || record.url || record.sourceUrl,
      reviewState: event ? event.reviewState : "recorded-unverified",
      verificationDepth: checked ? summaryDepth : "not-read",
      ...
      sourceUrl: registrySource?.url ?? null,
      sourceVersion: registrySource?.version?.label ?? null,
      scope: scopes,
      boundary: event?.boundary || registrySource?.version?.label || "Registry-resolved source record.",
    };
  });
  const paperReviews = (paper) => registryReviews(paper.verification?.sources, { where: `paper ${paper.id}`, expectedDoi: paper.doi });
  // A correction notice is its own document, joined to the registry by URL. Routing its reviews
  // through the same resolver closes the last un-joined path: an inline notice copy can no longer
  // mint an "independently-rechecked" state, because that state now comes only from a resolvable
  // registry event that validateIndependentEvent has checked.
  const noticeReviews = (paper, event) => registryReviews(event.reviews, { where: `paper ${paper.id} notice ${event.noticeType}` });
    ...
    for (const event of paper.versionEvents || []) {
      ...
      edges.push({
        ...
        ...reviewFrom({
          ...
          records: noticeReviews(paper, event),
  ...
  for (const link of network.methodLinks || []) {
    ...
    // A method asserts two opposite things — what it measures and what it cannot distinguish —
    // and each is promoted by its own scope. Pointing both at the same reading is the method
    // analogue of the wrong-figure borrow: it lets the passage read for the boundary promote the
    // measurement claim, or vice versa. The two assertions must therefore cite different scopes.
    // Binding a method assertion to its own intrinsic evidence location is not otherwise possible
    // — unlike a paper claim, a method assertion has no figure of its own — so this is the
    // available structural guard; which reading supports which assertion remains a curated
    // judgement the validator trusts, exactly like a support mode.
    const measuresScope = link.assertionScopes?.MEASURES || null;
    const boundaryScope = link.assertionScopes?.CANNOT_DISTINGUISH || null;
    if (measuresScope && boundaryScope && measuresScope === boundaryScope) {
      problems.push(`method link ${method.id}: MEASURES and CANNOT_DISTINGUISH both cite scope ${JSON.stringify(measuresScope)}; the same reading cannot separately establish what the assay measures and what it cannot distinguish`);
    }
