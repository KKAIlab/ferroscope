  ...
  for (const axis of METHOD_DECISION_AXES) {
    ...
    if (field.status === "source-checked") {
      ...
      for (const [ei, entry] of evidence.entries()) {
        ...
        // A clause is a real span of the claim, written out. Three gaming patterns are rejected:
        // an ellipsis (a truncated copy of the whole value, not a clause), a "(clause N)" index
        // suffix (which was used to make two identical truncations pass the uniqueness check),
        // and — for an EXPLICIT claim — a fragment that is not literally present in the field
        // value, because "explicit" asserts the source states it, so a paraphrase must instead
        // be labelled derived or analytical-inference.
        const rawFragment = entry.claimFragment;
        if (!rawFragment) problems.push(`${ew}: a checked field must name the claim fragment (clause) this evidence supports`);
        else {
          if (String(rawFragment).includes("…")) problems.push(`${ew}: claimFragment ${JSON.stringify(rawFragment)} is a truncated copy of the value (contains an ellipsis), not a written-out clause`);
          if (/\(clause\s*\d+\)\s*$/i.test(rawFragment)) problems.push(`${ew}: claimFragment ${JSON.stringify(rawFragment)} ends in a "(clause N)" index; a clause must name its content, and the suffix was used to disguise a duplicate`);
          if (fragments.has(rawFragment)) problems.push(`${ew}: duplicate claimFragment ${JSON.stringify(rawFragment)}; each evidence row must map a distinct clause`);
          else fragments.add(rawFragment);
          const fragmentText = String(rawFragment).replace(/\s*\(clause\s*\d+\)\s*$/i, "").replace(/…/g, "").trim().toLowerCase();
          if (entry.supportMode === "explicit" && field.value && fragmentText && !String(field.value).toLowerCase().includes(fragmentText)) {
            problems.push(`${ew}: marked explicit, but its claim fragment is not a literal span of the field value; an explicit claim quotes the source, so relabel it derived or analytical-inference`);
          }
        }
