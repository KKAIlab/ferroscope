...
// Days in each month, indexed 1-12. February is resolved against the actual year rather
// than assumed 28, so a leap day is accepted only in a leap year — all without ever
// constructing a Date, which would reintroduce the timezone shift this module exists to avoid.
...
export function parseCalendarDate(raw) {
  ...
  if (iso) {
    ...
    // A day component that names an impossible date (31 February, 29 February in a common
    // year) is rejected outright rather than stored: an ingestion source that hands back a
    // malformed date is a quality signal, not something to silently normalise to month
    // precision, which would hide the defect behind a plausible-looking record.
    if (day) return isRealCalendarDate(year, month, day) ? { date: `${year}-${pad(month)}-${pad(day)}`, precision: "day" } : null;
...
export function formatCalendarDate(value) {
  ...
  // The same real-calendar test as the parser, so a stored "2025-02-31" is escaped as raw
  // text by the caller rather than shown as a confident "31 Feb 2025".
  if (!label || !isRealCalendarDate(year, month, day)) return null;
  return `${day} ${label} ${year}`;
}

// Crossref returns date-parts as integers, which are already calendar components.
// The same rejection policy as parseCalendarDate: date-parts naming an impossible
// day or month fail the whole parse rather than being stored — "2026-02-31" satisfies
// an ISO-shaped regex, so without this check a deposit error would ride through every
// downstream date contract as a date that does not exist.
export function calendarDateFromParts(parts = []) {
  ...
  return isRealCalendarDate(year, month, day) ? { date: `${year}-${pad(month)}-${pad(day)}`, precision: "day" } : null;
}
...
// A record whose date did not parse carries `date: null`. The public data contract in
// scripts/validate-data.mjs requires an ISO calendar date on every automated record, and
// that check runs in the refresh workflow *before* the fetched data is committed. One
// undated record from any upstream source — a Crossref preprint with no published date, a
// ClinicalTrials.gov study with no posted date — therefore fails validation and aborts the
// whole refresh, so the freshly fetched dataset is discarded and the live site stops
// updating. Undated records are dropped at ingestion instead, and the count is logged
// rather than hidden, so one malformed upstream record cannot freeze every other record's
// update.
export const ISO_CALENDAR_DATE = /^(\d{4})-(\d{2})-(\d{2})$/;

// The shape test and the reality test together: "2026-02-31" matches the regex but is
// not a date. The partition and the validator share this predicate, so a record the
// generator publishes can never be one the validator rejects.
export function isUsableCalendarDate(value) {
  const match = ISO_CALENDAR_DATE.exec(String(value || ""));
  return Boolean(match) && isRealCalendarDate(match[1], match[2], match[3]);
}

export function partitionByUsableDate(records = []) {
  const dated = [];
  const undated = [];
  for (const record of records) {
    (isUsableCalendarDate(record?.date) ? dated : undated).push(record);
  }
  return { dated, undated };
}

// A quiet run and a broken upstream both arrive as a well-formed empty answer, and the
// bytes alone cannot tell them apart. Two facts separate them honestly: how much the
// source published last run, and whether it was already failing for emptiness. The
// repository's own history shows the general PubMed query legitimately oscillating
// between one and zero published records across consecutive runs, so a zero-yield is a
// failure only when the previous run published enough records that losing every one in
// a single cycle is implausible — or when the source was already failed with class
// "empty-result", so a chronic outage cannot age past the retention window and then
// flip back to a false "ok, 0 records". An unrelated past failure (a timeout) does not
// make a quiet run a failure.
export const EMPTY_RESULT_MIN_PREVIOUS = 5;

export function emptyHarvestIsFailure({ previousCount = 0, previousErrorClass = null, minPrevious = EMPTY_RESULT_MIN_PREVIOUS } = {}) {
  return previousCount >= minPrevious || previousErrorClass === "empty-result";
}
...
export function mergeCanonicalRecords(records) {
  ...
  // The first usable date wins: a legacy retained record can carry an ISO-shaped but
  // impossible date written under the older shape-only contract, and copying it onto
  // the merged card would get the whole card dropped by the generator's date partition
  // even though another route carries a real date.
  const dateCandidates = [automated[0]?.date, freshest.date, primary.date];
  merged.date = dateCandidates.find((value) => isUsableCalendarDate(value)) || dateCandidates.find(Boolean) || null;
...
// stale until they pass the maximum age, at which point that source publishes nothing
// and its "failed" status row is published in its place — the failure, its error class
// and the date of the last success stay visible in the freshness dialog while the
// other sources keep updating.
...
export function retainOnFailure({ sourceName, previousItems = [], lastSuccessAt, attemptedAt, errorClass, maxAgeDays = STALE_MAX_AGE_DAYS }) {
  const retained = [];
  // A previous dataset that parses but is not an array (a truncated write, a hand edit)
  // must degrade into the ordinary nothing-retained path, not crash the generator —
  // a crashed generator publishes nothing and freezes every source at once.
  for (const item of Array.isArray(previousItems) ? previousItems : []) {
    // A corrupt element (a literal null, a bare string) is skipped for the same reason
    // the non-array shape is: it must degrade, not crash the run for every source.
    ...
    // The stored canonical identity is dropped so the merge and the validator re-derive
    // it under the current rules. A retained record carrying an identity minted by an
    // older rule set can otherwise collide with the fresh copy of the same study and
    // fail the uniqueness contract.
    const { canonicalId: _cachedId, canonicalIdKind: _cachedKind, ...cachedFields } = item;
    retained.push({
      ...cachedFields,
      sourceName,
  ...
  // Age is measured from whichever success date the retained routes actually carry, not from
  // the function argument alone. When a merged record's secondary route fails, the caller has
  // no top-level `lastSuccessAt` for it, but the cached route does; using only the argument
  // treated that route as infinitely old and discarded a record that was in fact current.
  const routeSuccessAt = lastSuccessAt
    || retained.map((item) => item.lastSuccessAt).filter(Boolean).sort().at(-1)
    || null;
  const ageDays = routeSuccessAt ? daysBetween(routeSuccessAt, attemptedAt) : Number.POSITIVE_INFINITY;
  ...
  return {
    ...
    status: {
      ...
      note: usable
        ? `This source failed on the last attempt (${errorClass}). The ${retained.length} records it returned on ${String(routeSuccessAt).slice(0, 10)} are retained and marked stale; they are ${ageDays} days old against a ${maxAgeDays}-day limit.`
