...
// A document's class bounds which surfaces it can plausibly expose. A bibliographic metadata
// record, a bare citation index or a vendor catalogue page has no figures, methods or results
// to open, so such a source may not carry an experimental content surface. Without this, a
// checked event on a crossref-metadata-record source could carry a scope that merely *declares*
// surfaceType "figure-caption", and a paper's figureAudit could point at it to promote a figure
// claim to source-checked that no reader ever opened — the edge-level content-surface allowlist
// trusts the scope's self-declared surfaceType, so honesty of that surfaceType has to be
// enforced here, at the source. Classes that ARE the primary document (author manuscript,
// version of record, raw data) carry any surface and are intentionally absent from this map
// (unconstrained); only record/notice/vendor classes, which cannot hold an experiment, are bound.
export const DOCUMENT_CLASS_SURFACES = {
  "crossref-metadata-record": ["metadata-record"],
  "pubmed-record": ["metadata-record", "abstract-text"],
  "publisher-correction-notice": ["correction-text", "metadata-record"],
  "author-correction-notice": ["correction-text", "metadata-record"],
  "editor-note": ["correction-text", "metadata-record"],
  "vendor-catalogue-page": ["vendor-description", "vendor-specifications", "vendor-faq"],
};

// A source's URL is the machine-checkable half of its documentClass claim. DOCUMENT_CLASS_SURFACES
// binds a scope's surfaceType to the documentClass, but that is worthless if the documentClass
// itself can be forged: relabelling a crossref metadata record as an (unconstrained) version-of-record
// while keeping its api.crossref.org URL would let it carry a figure-caption scope and promote a
// figure edge to source-checked on metadata no reader opened. A metadata-API endpoint is
// definitionally a metadata record, so its URL pins its class. (The converse — a fabricated
// primary-document source with a plausible publisher URL claiming a reading nobody performed — is
// the trust boundary documented in DELIVERY_AUDIT_ROUND7.md §6, and is not machine-disprovable.)
//
// This list covers the two pure-metadata APIs the project actually ingests from (crossref, pubmed);
// both are pinned, so the check is complete for the current data. Extend it — as a per-batch task —
// only when ingesting from another PURELY-metadata API (e.g. OpenAlex, Semantic Scholar). Do NOT add
// hosts that also serve full text (Europe PMC fullTextXML, NCBI E-utilities efetch), because those
// legitimately back content surfaces and pinning them to a metadata class would be wrong.
export const URL_PINNED_DOCUMENT_CLASS = [
  [/^https:\/\/api\.crossref\.org\//i, "crossref-metadata-record"],
  [/^https:\/\/pubmed\.ncbi\.nlm\.nih\.gov\//i, "pubmed-record"],
];
...
export function validateRegistry(registry, { owner = registry?.owner } = {}) {
  ...
  for (const [index, source] of (registry.sources || []).entries()) {
    ...
    if (!source.documentClass) problems.push(`${where}: a source needs a documentClass`);
      ...
      const pin = URL_PINNED_DOCUMENT_CLASS.find(([re]) => re.test(source.url || ""));
      if (pin && source.documentClass !== pin[1]) {
        problems.push(`${where}: a ${JSON.stringify(source.url)} URL is a ${pin[1]} and cannot be declared documentClass ${JSON.stringify(source.documentClass)}; a metadata-API endpoint may not be relabelled as a primary document`);
      }
    }
    ...
    for (const [si, scope] of (source.scopes || []).entries()) {
      ...
      if (!SURFACE_TYPES.includes(scope.surfaceType)) problems.push(`${sw}: surfaceType must be one of ${SURFACE_TYPES.join(", ")}`);
        ...
        const allowed = DOCUMENT_CLASS_SURFACES[source.documentClass];
        if (allowed && !allowed.includes(scope.surfaceType)) {
          problems.push(`${sw}: a ${source.documentClass} source cannot expose a ${JSON.stringify(scope.surfaceType)} surface (allowed: ${allowed.join(", ")}); a metadata, notice or vendor record holds no experiment to open`);
        }
      }
