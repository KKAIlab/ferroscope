...
const [papers, labs, labsEn, links, methods, network, claims, manifest] = await Promise.all([
  read("papers-en.json"), read("labs.json"), read("labs-en.json"), read("lab-paper-links.json"),
  read("methods.json"), read("knowledge-network.json"), read("paper-claims.json"), read("schema-versions.json"),
]);
...
const graph = buildGraph({ papers, labs, labsEn, links, methods, network, claims });
